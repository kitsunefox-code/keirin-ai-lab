...
def main() -> None:
    ...
    with connect() as conn:
        ...
        if session:
            if session["session_date"] == today and session["config"].get("style") == "original":
                print(json.dumps({"ok": True, "action": "already-running", "session_id": session["id"]}, ensure_ascii=False))
                return
